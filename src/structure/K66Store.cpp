...
K66Store::K66Store() : edges_in_circles_(Constants::MAX_SIZE, Constants::MAX_SIZE) {}
...
bool K66Store::createsKst(const Graph& adj, int u, int v) const
{
  int m = adj.m;
  int n = adj.n;
                            ...
                            if (adj[u5][v] && adj[u5][v2] && adj[u5][v3] && adj[u5][v4])
                            {
                              for (int v5 = v4 + 1; v5 < n; ++v5)
                              {
                                    ...
                                    adj[u5][v5])
                                {
                                  ...
                                  {
                                    if (u6 == u) continue;
                                    if (adj[u6][v] && adj[u6][v2] && adj[u6][v3] && adj[u6][v4] &&
                                        adj[u6][v5])
                                    {
                                      for (int v6 = v5 + 1; v6 < n; ++v6)
                                      {
                                        if (v6 == v) continue;
                                        if (adj[u][v6] && adj[u2][v6] && adj[u3][v6] &&
                                            adj[u4][v6] && adj[u5][v6] && adj[u6][v6])
                                        {
                                          return true;
                                        }
                                      }
...
void K66Store::storeKst(const Graph& adj, int u, int v)
{
  ...
  for (int i = 0; i < u_cands_size; ++i)
  {
    for (int j = i + 1; j < u_cands_size; ++j)
    {
      for (int k = j + 1; k < u_cands_size; ++k)
      {
        ...
        {
          for (int m = l + 1; m < u_cands_size; ++m)
          {
            v_cands_size = 0;
            ...
            for (int node_v = 0; node_v < adj.n; ++node_v)
            {
              ...
              if (adj[u][node_v] && adj[u_cands[i]][node_v] && adj[u_cands[j]][node_v] &&
                  adj[u_cands[k]][node_v] && adj[u_cands[l]][node_v] && adj[u_cands[m]][node_v])
              {
              ...
              for (int b = a + 1; b < v_cands_size; ++b)
              {
                for (int c = b + 1; c < v_cands_size; ++c)
                {
                  for (int d = c + 1; d < v_cands_size; ++d)
                  {
                    ...
                    {
                      Kst<S_, T_> created(
                          {u, u_cands[i], u_cands[j], u_cands[k], u_cands[l], u_cands[m]},
...
void K66Store::reevalCircles(const Graph& adj)
{
  KstHelper::reevalCircles<S_, T_>(circles_, edges_in_circles_, adj);
}

bool K66Store::reflipCircle(Graph& adj, Probabilities& prob)
{
