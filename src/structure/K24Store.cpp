...
K24Store::K24Store() : edges_in_circles_(Constants::MAX_SIZE, Constants::MAX_SIZE) {}
...
bool K24Store::createsKst(const Graph& adj, int u, int v) const
{
  int m = adj.m;
  int n = adj.n;
...
void K24Store::storeKst(const Graph& adj, int u, int v)
{
  ...
  for (int i = 0; i < u_cands_size; ++i)
  {
    ...
    for (int a = 0; a < v_cands_size; ++a)
    {
      for (int b = a + 1; b < v_cands_size; ++b)
      {
        ...
        {
          Kst<S_, T_> created({u, u_cands[i]}, {v, v_cands[a], v_cands[b], v_cands[c]});
...
void K24Store::reevalCircles(const Graph& adj)
{
  KstHelper::reevalCircles<S_, T_>(circles_, edges_in_circles_, adj);
}

bool K24Store::reflipCircle(Graph& adj, Probabilities& prob)
{
