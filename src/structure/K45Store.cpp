...
K45Store::K45Store() : edges_in_circles_(Constants::MAX_SIZE, Constants::MAX_SIZE) {}
...
bool K45Store::createsKst(const Graph& adj, int u, int v) const
{
  int m = adj.m;
  int n = adj.n;
...
void K45Store::storeKst(const Graph& adj, int u, int v)
{
...
void K45Store::reevalCircles(const Graph& adj)
{
  KstHelper::reevalCircles<S_, T_>(circles_, edges_in_circles_, adj);
}

bool K45Store::reflipCircle(Graph& adj, Probabilities& prob)
{
