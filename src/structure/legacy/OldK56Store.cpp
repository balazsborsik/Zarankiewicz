...
void OldK56Store::clear()
{
  circles_.clear();
  edges_in_circles_.reset();
}

bool OldK56Store::createsKst(const Graph& adj, int u, int v) const
{
  int m = adj.m;
  int n = adj.n;
...
void OldK56Store::storeKst(const Graph& adj, int u, int v)
{
  int m = adj.m;
  int n = adj.n;
  for (int u2 = 0; u2 < m; ++u2)
...
void OldK56Store::reevalCircles(const Graph& adj)
{
  circles_.erase(
...
bool OldK56Store::reflipCircle(Graph& adj, Probabilities& prob)
{
