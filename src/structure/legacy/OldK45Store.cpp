...
void OldK45Store::clear()
{
  circles_.clear();
  edges_in_circles_.reset();
}

bool OldK45Store::createsKst(const Graph& adj, int u, int v) const
{
  int m = adj.m;
  int n = adj.n;
...
void OldK45Store::storeKst(const Graph& adj, int u, int v)
{
  int m = adj.m;
  int n = adj.n;
  for (int u2 = 0; u2 < m; ++u2)
                      ...
                      for (int v4 = v3 + 1; v4 < n; ++v4)
                      {
                        if (adj[u][v4] && adj[u2][v4] && adj[u3][v4] && adj[u4][v4])
                        {
                          for (int v5 = v4 + 1; v5 < n; ++v5)
                          {
                            if (adj[u][v5] && adj[u2][v5] && adj[u3][v5] && adj[u4][v5])
                            {
                              Kst<4, 5> created({u, u2, u3, u4}, {v, v2, v3, v4, v5});
                              for (int i = 0; i < 4; ++i)
                              {
                                for (int j = 0; j < 5; ++j)
                                {
...
void OldK45Store::reevalCircles(const Graph& adj)
{
  circles_.erase(
      std::remove_if(circles_.begin(), circles_.end(),
                     [&adj, this](const Kst<4, 5>& circle)
                     {
                       for (int i = 0; i < 4; ++i)
                       {
                         for (int j = 0; j < 5; ++j)
...
bool OldK45Store::reflipCircle(Graph& adj, Probabilities& prob)
{
