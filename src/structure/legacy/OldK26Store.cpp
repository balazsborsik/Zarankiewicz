...
void OldK26Store::clear()
{
  circles_.clear();
  edges_in_circles_.reset();
}

bool OldK26Store::createsKst(const Graph& adj, int u, int v) const
{
  int m = adj.m;
  int n = adj.n;
...
void OldK26Store::storeKst(const Graph& adj, int u, int v)
{
  int m = adj.m;
  int n = adj.n;
  for (int u2 = 0; u2 < m; ++u2)
...
void OldK26Store::reevalCircles(const Graph& adj)
{
  circles_.erase(
...
bool OldK26Store::reflipCircle(Graph& adj, Probabilities& prob)
{
