...
void OldK34Store::clear()
{
  circles_.clear();
  edges_in_circles_.reset();
}

bool OldK34Store::createsKst(const Graph& adj, int u, int v) const
{
  int m = adj.m;
  int n = adj.n;
...
void OldK34Store::storeKst(const Graph& adj, int u, int v)
{
  int m = adj.m;
  int n = adj.n;
  for (int u2 = 0; u2 < m; ++u2)
...
void OldK34Store::reevalCircles(const Graph& adj)
{
  circles_.erase(
...
bool OldK34Store::reflipCircle(Graph& adj, Probabilities& prob)
{
