...
void OldK24Store::clear()
{
  circles_.clear();
  edges_in_circles_.reset();
}

bool OldK24Store::createsKst(const Graph& adj, int u, int v) const
{
  int m = adj.m;
  int n = adj.n;
...
void OldK24Store::storeKst(const Graph& adj, int u, int v)
{
  int m = adj.m;
  int n = adj.n;
  for (int u2 = 0; u2 < m; ++u2)
          ...
          for (int v3 = v2 + 1; v3 < n; ++v3)
          {
            if (adj[u][v3] && adj[u2][v3])
            {
              for (int v4 = v3 + 1; v4 < n; ++v4)
              {
                if (adj[u][v4] && adj[u2][v4])
                {
                  Kst<2, 4> created({u, u2}, {v, v2, v3, v4});
                  for (int i = 0; i < 2; ++i)
                  {
                    for (int j = 0; j < 4; ++j)
                    {
...
void OldK24Store::reevalCircles(const Graph& adj)
{
  circles_.erase(
      std::remove_if(circles_.begin(), circles_.end(),
                     [&adj, this](const Kst<2, 4>& circle)
                     {
                       for (int i = 0; i < 2; ++i)
                       {
                         for (int j = 0; j < 4; ++j)
...
bool OldK24Store::reflipCircle(Graph& adj, Probabilities& prob)
{
