...
void OldK36Store::clear()
{
  circles_.clear();
  edges_in_circles_.reset();
}

bool OldK36Store::createsKst(const Graph& adj, int u, int v) const
{
  int m = adj.m;
  int n = adj.n;
                ...
                if (adj[u][v3] && adj[u2][v3] && adj[u3][v3])
                {
                  for (int v4 = v3 + 1; v4 < n; ++v4)
                  {
                    if (adj[u][v4] && adj[u2][v4] && adj[u3][v4])
                    {
                      ...
                      {
                        if (adj[u][v5] && adj[u2][v5] && adj[u3][v5])
                        {
...
void OldK36Store::storeKst(const Graph& adj, int u, int v)
{
  int m = adj.m;
  int n = adj.n;
  for (int u2 = 0; u2 < m; ++u2)
                      ...
                      for (int v5 = v4 + 1; v5 < n; ++v5)
                      {
                        if (adj[u][v5] && adj[u2][v5] && adj[u3][v5])
                        {
                          for (int v6 = v5 + 1; v6 < n; ++v6)
                          {
                            if (adj[u][v6] && adj[u2][v6] && adj[u3][v6])
                            {
                              Kst<3, 6> created({u, u2, u3}, {v, v2, v3, v4, v5, v6});
                              for (int i = 0; i < 3; ++i)
                              {
                                for (int j = 0; j < 6; ++j)
                                {
...
void OldK36Store::reevalCircles(const Graph& adj)
{
  circles_.erase(
      std::remove_if(circles_.begin(), circles_.end(),
                     [&adj, this](const Kst<3, 6>& circle)
                     {
                       for (int i = 0; i < 3; ++i)
                       {
                         for (int j = 0; j < 6; ++j)
...
bool OldK36Store::reflipCircle(Graph& adj, Probabilities& prob)
{
