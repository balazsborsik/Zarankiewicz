...
void OldK33Store::clear()
{
  circles_.clear();
  edges_in_circles_.reset();
}

bool OldK33Store::createsKst(const Graph& adj, int u, int v) const
{
  int m = adj.m;
  int n = adj.n;
...
void OldK33Store::storeKst(const Graph& adj, int u, int v)
{
  int m = adj.m;
  int n = adj.n;
  for (int u2 = 0; u2 < m; ++u2)
...
void OldK33Store::reevalCircles(const Graph& adj)
{
  circles_.erase(
...
bool OldK33Store::reflipCircle(Graph& adj, Probabilities& prob)
{
